...
def fr_foot_airtime_reward(
    ...
    # print("Airtime reward; ", reward)
    return torch.sum(reward, dim=1)


def fr_foot_height_reward(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, target_height: float) -> torch.Tensor:
    foot = env.scene[asset_cfg.name]
    z = foot.data.root_pos_w[:, 2]
    reward = torch.clamp(z - target_height, min=0.0, max=0.1)
    # return torch.sum(reward, dim=1)
...
def foot_slip_penalty(
    ...
    # print("In foot slip penalty: ", asset_cfg.body_ids) [13, 14, 15, 16]
