...
import isaaclab_tasks.manager_based.locomotion.balance.mdp as mdp
...
class RewardsCfg:
    ...
    base_motion = RewTerm(
        func=mdp.base_motion_penalty, weight=-0.5, params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["fl_foot", "hl_foot", "hr_foot"])}
    )
    base_orientation = RewTerm(
        func=mdp.base_orientation_penalty, weight=-0.1, params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["fl_foot", "hl_foot", "hr_foot"])}
    )
    ...
    foot_slip = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["fl_foot", "hl_foot", "hr_foot"]),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["fl_foot", "hl_foot", "hr_foot"]),
            "threshold": 1.0,
        },
    )
    joint_acc = RewTerm(
    ...
    # Penalties too low -1 rn, when at least one of the support feet isn't touching the ground
    ...
    # Reward for body staying at max height
...
@configclass
class SpotBalanceEnvCfg(ManagerBasedRLEnvCfg):
    ...
    #     """Post initialization."""
        # General settings
        self.decimation = 4
        self.episode_length_s = 10.0  # Shorter episodes for kicking
...
class SpotBalanceEnvCfg_PLAY(SpotBalanceEnvCfg):
